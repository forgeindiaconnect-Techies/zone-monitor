...
// Get team-wise visitor counts
router.get('/team-visitors', async (req, res) => {
  try {
    const { branchId, filter } = req.query;
    
    // 1. Build match stage for branch
    const matchStage = {};
    ...
    // 2. Build match stage for date filter
    const now = new Date();
    // Default to 'Today' if no filter provided
    const timeFilter = filter || 'Today';
    
    // We filter using the 'visitDate' string field (YYYY-MM-DD)
    if (timeFilter === 'Today') {
      const todayStr = now.toISOString().split('T')[0];
      matchStage.visitDate = todayStr;
    } else if (timeFilter === 'This Week') {
      const startOfWeek = new Date(now.setDate(now.getDate() - now.getDay()));
      const startStr = startOfWeek.toISOString().split('T')[0];
      matchStage.visitDate = { $gte: startStr };
    ...
    // Map to friendly format and handle null teams
    const formattedData = teamCounts.map(t => ({
      team: t._id || 'General',
      count: t.count
    }));
